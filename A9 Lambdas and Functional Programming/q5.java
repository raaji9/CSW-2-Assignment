import java.util.*;

interface StringPredicate {
    boolean test(String str);
}

public class q5 {
    public static void main(String[] args) {
        List<String> strings = new ArrayList<>();
        strings.add("apple");
        strings.add("Banana");
        strings.add("orange");
        strings.add("kiwi");
        strings.add("Pineapple");

        List<String> filteredStrings = filterStrings(strings, str -> !startsV(str));
        printStr(filteredStrings);
    }

    public static List<String> filterStrings(List<String> strings, StringPredicate predicate) {
        List<String> filtered = new ArrayList<>();
        for (String str : strings) {
            if (predicate.test(str)) {
                filtered.add(str.toUpperCase());
            }
        }
        return filtered;
    }

    public static boolean startsV(String str) {
        String firstLetter = str.substring(0, 1).toUpperCase();
        return firstLetter.matches("[AEIOU]");
    }

    public static void printStr(List<String> strings) {
        for (String str : strings) {
            System.out.println(str);
        }
    }
}
